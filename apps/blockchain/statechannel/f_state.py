...
class F_State(UCWrappedFunctionality):
    ...
    def move_to_next_round(self, scheduled_clock_round):
        ...
        if curr_round < (scheduled_clock_round + 3*self.delta):
            ...
            self.write_and_wait_expect(
                ch='f2w', msg=((self.sid, 'F_Wrapper'), ('schedule', 'move_to_next_round', (scheduled_clock_round,), rounds_left_to_wait)),
    ...
    def party_input(self, pid, inp):
        ...
        if self.first_input_this_round:
            ...
            self.write_and_wait_expect(
                ch='f2w', msg=((self.sid, 'F_Wrapper'), ('schedule', 'move_to_next_round', (curr_clock_round,), 3*self.delta)),
                read='w2f', expect=((self.sid, 'F_Wrapper'), ('OK',))
            )
